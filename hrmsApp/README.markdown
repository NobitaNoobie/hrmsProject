...
1. forms are a flexible mechanism for collecting user input. 
2. forms ar also relatively secure way of sharing data  with the server, as they allow us to send data in POST requests with cross-site request forgery protection.

Django forms are of various types:
1. RegularForm: A form that you define manually, without using a model.
    ...
    * each field is defined using a form field class from the forms module.

2. ModelForm : It is a form that is automatically generated from a Django model. It can be used to create, update, and delete model instances.

3. FormSet: A django formset is a collection of forms that you can use to handle multiple instances of a form. You can use formsets to handle multiple instances of a model or multiple instances of a regular form. FormSet is used to create a set of forms that can be used to gather data for multiple instances of a model.

Customization of Django form:
        from django import forms
        ...
        #form validation
            ...
            if num_words < 4:
                raise forms.ValidationError("Message must be atleast 4 words long.")
            return message

CROSS-SITE REQUEST FORGERY(CSRF): verifys whether request is from an authenticated user, and not from a malacious website or a script. 

user submits -> csrf_token middleware compares the submitted token to the one generated by Django. -> token matches: request is processed -> token does not match: Django raises a CSRF token missing or incorrect error and aborts the request.
